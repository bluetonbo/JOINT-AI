...
if not st.session_state.authenticated:
    ...
    with center:
        st.markdown("<br><br><h2 style='text-align: center; color: #10b981;'>JOINT AI SYSTEM ACCESS</h2>", unsafe_allow_html=True)
    ...
    with tab2:
        ...
        if st.session_state['sim_pred_tq'] is not None:
            ...
            df_sim_excel = pd.DataFrame({
                "시뮬레이션 항목": ["입력 코킹 거리 (Caulking_Distance)", "입력 스터드 센터 (Stud_Center)", "입력 에이징 상태 (Aging_Status)", 
                               "AI 예측 토크 (Torque)", "AI 예측 내구 수명 (Endurance)", "예측 모델 데이터 신뢰도 (Confidence)"],
                "엔지니어 설정값 및 품질 예측치": [f"{ev[0]:.2f} mm", f"{ev[1]:.2f} mm", "Aged (에이징)" if ev[2] == 1 else "Unaged (미적용)", 
                                               f"{st.session_state['sim_pred_tq']:.2f} Nm", f"{st.session_state['sim_pred_ed']:,.0f} Cycles", f"{st.session_state['sim_confidence']:.1f} %"]
            })
